...
class WEC_Public {
	...
	public function render_calendar_shortcode( $atts ) {
		...
		ob_start();
		?>
		<div class="wec-wrapper" style="display: flex; flex-wrap: wrap; gap: 20px; background: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); max-width: 1200px; margin: 0 auto;">
			<div id="wec-calendar-container" style="flex: 1 1 65%; min-width: 300px;"></div>
			
			<div id="wec-event-list-sidebar" style="flex: 1 1 30%; min-width: 250px; border-left: 1px solid #eee; padding-left: 20px;">
		...
		<div id="wec-event-modal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.5); z-index:9999;">
			<div style="position:relative; top:50%; left:50%; transform:translate(-50%, -50%); background:#fff; padding:20px; border-radius:8px; max-width:400px; width:90%;">
				...
				<img id="wec-modal-image" src="" style="width:100%; max-width:300px; height:auto; display:none; margin-bottom:15px; border-radius:4px;" />
				<p><strong>Waktu:</strong> <span id="wec-modal-time"></span></p>
				<div id="wec-modal-desc"></div>
				<a id="wec-modal-link" href="#" style="display:inline-block; margin-top:10px; color:#0073aa;">Lihat Detail &rarr;</a>
			</div>
		</div>
		<?php
		return ob_get_clean();
